...
from opendivine.types import Hexagram, Trigram
...
def load_iching_corpus() -> tuple[list[Hexagram], list[Trigram]]:
    ...
    if _hexagrams is None:
        hex_file = files("opendivine.corpora.iching") / "data.json"
        tri_file = files("opendivine.corpora.iching") / "trigrams.json"
...
def get_hexagram_image_path(hexagram: Hexagram) -> Path:
    ...
    return Path(str(files("opendivine.corpora.iching") / "images" / hexagram.image_path))
